...
LEVEL_WIN_MESSAGE = (
    "win"  # checkForGameOver() returns this value if the level has been won
)
...
ROLLING_COMPLETE = (
    0  # unix timestamp of when the rolling of the mat will have completed
)
LAST_PLATE_CLEARING = 0  # unix timestamp of the last time the plates were cleared
LAST_GAME_OVER_CHECK = (
    0  # unix timestamp when we last checked for the Game Over or You Win messages
)
...
def getGameRegion():
    ...
    GAME_REGION = (
        ...
        800,
        600,
    )  # the game screen is always 800 x 600
    logging.debug("Game region found: %s" % (GAME_REGION,))
    logging.debug("Game region found: %s" % (region,))
...
def setupCoordinates():
    ...
    INGRED_COORDS = {
        SHRIMP: (GAME_REGION[0] + 46, GAME_REGION[1] + 420),
        RICE: (GAME_REGION[0] + 110, GAME_REGION[1] + 420),
        NORI: (GAME_REGION[0] + 46, GAME_REGION[1] + 490),
        ROE: (GAME_REGION[0] + 110, GAME_REGION[1] + 490),
        SALMON: (GAME_REGION[0] + 46, GAME_REGION[1] + 556),
        UNAGI: (GAME_REGION[0] + 110, GAME_REGION[1] + 556),
    }
    PHONE_COORDS = (GAME_REGION[0] + 730, GAME_REGION[1] + 456)
    TOPPING_COORDS = (GAME_REGION[0] + 675, GAME_REGION[1] + 340)
    RICE1_COORDS = (GAME_REGION[0] + 675, GAME_REGION[1] + 366)
    RICE2_COORDS = (GAME_REGION[0] + 676, GAME_REGION[1] + 350)
 
    NORMAL_DELIVERY_BUTTON_COORDS = (GAME_REGION[0] + 610, GAME_REGION[1] + 366)
    MAT_COORDS = (GAME_REGION[0] + 250, GAME_REGION[1] + 476)
 
    ORDER_BUTTON_COORDS = {
        SHRIMP: (GAME_REGION[0] + 610, GAME_REGION[1] + 276),
        UNAGI: (GAME_REGION[0] + 716, GAME_REGION[1] + 276),
        NORI: (GAME_REGION[0] + 610, GAME_REGION[1] + 346),
        ROE: (GAME_REGION[0] + 716, GAME_REGION[1] + 346),
        SALMON: (GAME_REGION[0] + 610, GAME_REGION[1] + 416),
    }
...
def navigateStartGameMenu():
    ...
    while (
        True
    ):  # loop because it could be the blue or pink Play button displayed at the moment.
        pos = pyautogui.locateCenterOnScreen(
            imPath("play_button.png"),
        )
        if pos is not None:
            break
    ...
    region = None
    while region == None:
        logging.debug("Finding Continue button...")
        region = pyautogui.locateOnScreen("images/continue_button.png")
    if region != None:
        pyautogui.click(pyautogui.center(region), duration=0.35)
        logging.debug("Clicked on continue button...")
    pyautogui.sleep(0.25)
    region = None
    while region == None:
        logging.debug("Finding Continue button...")
        region = pyautogui.locateOnScreen("images/continue_button_2.png")
    if region != None:
        pyautogui.click(pyautogui.center(region), duration=0.35)
        logging.debug("Clicked on continue 2 button...")
    else:
        logging.debug("Region not found")
 
    # click on Skip
    region = None
    while region == None:
        logging.debug("Finding Skip button...")
        region = pyautogui.locateOnScreen("images/skip_button.png")
    pyautogui.click(pyautogui.center(region), duration=0.35)
    ...
    while region == None:
        logging.debug("Finding Continue button...")
        region = pyautogui.locateOnScreen("images/continue_button_3.png")
    if region != None:
        pyautogui.click(pyautogui.center(region), duration=0.35)
        logging.debug("Clicked on continue 3 button...")
    else:
        logging.debug("Region not found")
    pyautogui.click(pos, duration=0.25)
    logging.debug("Clicked on Continue button.")
    ...
    while True:
        ...
        for pos, order in added.items():
            ...
            if result is not None:
                ...
                logging.debug(
                    "Ingredients for %s not available. Putting on back order." % (order)
                )
 
        # Clear any finished plates.
        if (
            random.randint(1, 10) == 1
            or time.time() - PLATE_CLEARING_FREQ > LAST_PLATE_CLEARING
        ):
            clickOnPlates()
...
def clickOnPlates():
    ...
    # X-Coordinates: 100, 226,352 , 480, 606, 732
    for i in (100, 226, 352, 480, 606, 732):
        pyautogui.click(GAME_REGION[0] + i, GAME_REGION[1] + 258)
...
def getOrders():
    ...
    for orderType in ALL_ORDER_TYPES:
        allOrders = pyautogui.locateAllOnScreen(
            imPath("%s_order.png" % orderType),
            region=(GAME_REGION[0] + 24, GAME_REGION[1] + 48, 730, 80),
        )
...
def makeOrder(orderType):
    ...
    while (
        time.time() < ROLLING_COMPLETE
        and pyautogui.locateOnScreen(
            imPath("clear_mat.png"),
            region=(GAME_REGION[0] + 140, GAME_REGION[1] + 370, 220, 220),
        )
        is None
    ):
        time.sleep(0.1)
    ...
    pyautogui.click(
        MAT_COORDS, duration=0.25
    )  # click the rolling mat to make the order
    logging.debug("Made a %s order." % (orderType))
    ROLLING_COMPLETE = (
        time.time() + 1.5
    )  # give the mat enough time (1.5 seconds) to finish rolling before being used again
...
def findAndClickPlatesOnBelt():
    ...
    for color in ("pink", "blue", "red"):
        result = pyautogui.locateCenterOnScreen(
            imPath("%s_plate_color.png" % (color)),
            region=(GAME_REGION[0] + 426, GAME_REGION[1] + 380, 80, 110),
        )
        if result is not None:
            pyautogui.click(result)
            logging.debug(
                "Clicked on %s plate on belt at X: %s Y: %s"
                % (color, result[0], result[1])
            )
...
def orderIngredient(ingredient):
    ...
    logging.debug(
        "Ordering more %s (inventory says %s left)..."
    ...
    if ingredient == RICE and ORDERING_COMPLETE[RICE] is None:
        ...
        if pyautogui.locateOnScreen(
            imPath("cant_afford_rice.png"),
            region=(GAME_REGION[0] + 600, GAME_REGION[1] + 280, 150, 140),
            confidence=1,
        ):
            logging.debug("Can't afford rice. Canceling.")
            pyautogui.click(
                GAME_REGION[0] + 726, GAME_REGION[1] + 418, duration=0.25
            )  # click cancel phone button
            return
        ...
        if pyautogui.locateOnScreen(
            imPath("cant_afford_%s.png" % (ingredient)),
            region=(GAME_REGION[0] + 550, GAME_REGION[1] + 204, 230, 464),
            confidence=1,
        ):
...
def updateInventory():
    ...
    for ingredient in INVENTORY:
        if (
            ORDERING_COMPLETE[ingredient] is not None
            and time.time() > ORDERING_COMPLETE[ingredient]
        ):
...
def checkForGameOver():
    ...
    result = pyautogui.locateOnScreen(
        imPath("you_win.png"),
        region=(GAME_REGION[0] + 250, GAME_REGION[1] + 135, 300, 55),
    )
    if result is not None:
        pyautogui.click(GAME_REGION[0] + 400, GAME_REGION[1] + 470)
        return LEVEL_WIN_MESSAGE
 
    # check for "You Fail" message
    result = pyautogui.locateOnScreen(
        imPath("you_failed.png"),
        region=(GAME_REGION[0] + 200, GAME_REGION[1] + 165, 400, 50),
    )
    if result is not None:
